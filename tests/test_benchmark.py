...
def test_benchmark_case(case):
    ...
    spec = build_engineering_spec(
        case["prompt"],
        MockParser(parsed).parse(case["prompt"]),
    )
...
def test_dn50_is_not_silently_converted_to_50mm():
    case = next(c for c in BENCHMARK if c["id"] == "B06")
    spec = build_engineering_spec(
        case["prompt"],
        ParsedEngineeringIntent.model_validate(case["parsed_intent"]),
    )
...
def test_probably_aluminium_remains_hypothesis():
    case = next(c for c in BENCHMARK if c["id"] == "B09")
    spec = build_engineering_spec(
        case["prompt"],
        ParsedEngineeringIntent.model_validate(case["parsed_intent"]),
    )
...
def test_m6_clearance_hole_is_association_not_physical_fastener_or_derived_diameter():
    case = next(c for c in BENCHMARK if c["id"] == "B14")
    spec = build_engineering_spec(
        case["prompt"],
        ParsedEngineeringIntent.model_validate(case["parsed_intent"]),
    )
    ...
    assert spec.hole_diameter.state == EvidenceState.UNKNOWN
