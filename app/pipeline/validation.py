...
def validate_spec(spec: EngineeringSpec) -> ValidationReport:
    ...
    if (
        spec.pipe_diameter.value is not None
        and spec.wall_thickness.value is not None
        and spec.wall_thickness.value.value_mm
        >= spec.pipe_diameter.value.value_mm / 2
    ):
